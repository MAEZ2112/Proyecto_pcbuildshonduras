...
<section class="container-m row px-4 py-4">
    <h1 class="col-12">Detalle de Envio #{{orderid}}</h1>
</section>

<section class="container-m row px-4 py-4">
    <form action="index.php?page=Admin-Orders-Envio" method="post" class="col-12 col-m-8 offset-m-2">
        ...
        <div class="row my-2 align-center">
            <label class="col-12 col-m-3" for="shipping_status">Estado de Envio</label>
            <select class="col-12 col-m-9 {{shippingClass}}" id="shipping_status" name="shipping_status">
                <option value="PENDING"{{status_PENDING}}>PENDING</option>
                <option value="SHIPPING"{{status_SHIPPING}}>SHIPPING</option>
                <option value="DELIVERED"{{status_DELIVERED}}>DELIVERED</option>
            </select>
        </div>

        <div class="row my-2 align-center">
            <label class="col-12 col-m-3" for="shipping_status">Estado de Pedido: </label>
            <input class="col-12 col-m-9" type="text" id="order_status" readonly value="{{order_status}}" />
        </div>
        <br>
        <div class="row mt-4">
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        btnCancelar.addEventListener("click", (e) => {
            e.preventDefault();
            e.stopPropagation();
            window.location.assign("index.php?page=Admin-Orders-Envios");
        });
    });
</script>